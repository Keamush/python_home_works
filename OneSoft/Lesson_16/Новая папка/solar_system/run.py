...
def make_asteroids(base_planet: Turtle):
    """Make asteroids belt around space system"""
    return [Asteroid(base_planet, randint(400, 600)) for _ in range(500)]
...
def mainloop(make_system_planets: Callable, make_central_planet: Callable):
    ...
    asteroids = make_asteroids(base_planet)
    stars = planets + asteroids
    belt = make_belt_around_planet()
    while True:
        belt.clear()
        move_starts(stars, belt)
        window.update()
